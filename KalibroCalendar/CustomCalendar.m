...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint selected = CGPointMake(self.frame.size.width / 7 * (int) (self.selected % 7) + self.frame.size.width / 7 / 2, self.frame.size.height / 5 * (int) (self.selected % 5) + self.frame.size.height / 5 / 2);
    ...
    CGPoint order = CGPointMake(self.frame.size.width / 7 * (int) (self.selected % 7) + self.frame.size.width / 7 / 2, self.frame.size.height / 5 * (int) (self.selected % 5) + self.frame.size.height / 5 / 2 + 20);
    [self drawArc:order to:selected red:255.0f / 255.0f green:255.0f / 255.0f blue:0.0f / 255.0f radius:2.5f];
    
    srand(time(NULL));
    for (int i = 0; i < 4; i++) {
        int demo = rand() % 20 + 1;
        CGPoint order = CGPointMake(self.frame.size.width / 7 * (int) (demo % 7) + self.frame.size.width / 7 / 2, self.frame.size.height / 5 * (int) (demo % 5) + self.frame.size.height / 5 / 2 + 20);
        [self drawArc:order to:selected red:255.0f / 255.0f green:255.0f / 255.0f blue:0.0f / 255.0f radius:2.5f];
    }
    ...
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 7; j++) {
            if (nthDay >= self.startFrom && nthDay <= self.daysTotal) {
                UILabel *dayLabel = [[UILabel alloc] initWithFrame:CGRectMake(j * self.frame.size.width / 7, i * self.frame.size.height / 5, self.frame.size.width / 7, self.frame.size.height / 5)];
    ...
    for (int i = 0; i <= 5; i++) {
        CustomSeparator *separator = [[CustomSeparator alloc] initWithFrame:CGRectMake(0, self.frame.size.height / 5 * i, self.frame.size.width, 1)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    UILabel *distance = [[UILabel alloc] initWithFrame:CGRectMake(240, 0, 80, 40)];
