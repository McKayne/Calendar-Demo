...
@implementation PadCalendarSortController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    //self.sortBy = self.controller.sortBy;
    ...
    [header setText:@"Сортировать по"];
    [self.view addSubview:header];
    
    UITableView *sort = [[UITableView alloc] initWithFrame:CGRectMake(12.5, 55, 200, 145)];
    [sort setTableFooterView:[[UIView alloc] initWithFrame:CGRectZero]];
    [sort setSeparatorStyle:UITableViewCellSeparatorStyleNone];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        case 0:
            [cell.textLabel setText:@"Без сортировки"];
            break;
        case 1:
            [cell.textLabel setText:@"По дальности"];
            break;
        case 2:
            [cell.textLabel setText:@"По времени начала"];
            break;
        case 3:
            [cell.textLabel setText:@"По времени завершения"];
    }
...
- (void)performSort:(UITapGestureRecognizer *)recognizer {
    [self.view removeFromSuperview];
    [self.controller.dim.view removeFromSuperview];
    
    /*self.controller.sortBy = self.sortBy;
